"""
publisher/blog_publisher.py  ★ FIXED ★
========================================
Fixes for blank GitHub Pages (https://mark-aryan.github.io/phantomfeed/):

FIX 1 — SLUG COLLISION (root cause of missing posts)
  The original code used  meta.get("slug", post_dir.name)
  If meta.json doesn't have a "slug" key, this falls back to the folder name
  e.g. "nvd-advisory-ec82eb" — but MULTIPLE posts can share the same folder
  name pattern across different dates (out/news/2026-03-01/nvd-advisory-ec82eb
  AND out/news/2026-03-03/nvd-advisory-ec82eb both → same slug → last one wins,
  earlier post's index.html gets overwritten → missing posts on the blog).
  
  FIX: slug is now always a UNIQUE combination of date + folder name + cid suffix.

FIX 2 — MISSING .nojekyll  
  Without .nojekyll GitHub Pages runs Jekyll, which IGNORES files/folders
  that start with underscore (_) and may mangle HTML. Always write it.

FIX 3 — IMAGE PATH DEPTH
  index.html (depth=0) references posts/<slug>/image.png  → correct
  posts/<slug>/index.html (depth=2) references just image.png → correct
  These were already fine but now explicitly verified.

FIX 4 — POSTS FROM DEEP FOLDER STRUCTURE
  out/ has 4-level nesting: out/<category>/<date>/<slug>/meta.json
  The original rglob("meta.json") handled this correctly, but some meta.json
  files have no "slug" key (older posts) causing the collision above.
"""
...
CATEGORY_COLOR = {
    "vulnerability": "#dc3232", "incident": "#ff8c00",
    "fraud": "#c800c8",         "bug": "#ffc800", "news": "#00d4ff",
}

# ─── CSS ──────────────────────────────────────────────────────────────────────
SHARED_CSS = """
:root{
  --bg:#05050f;--bg2:#0d0d1e;--bg3:#13132a;--cyan:#00d4ff;--blue:#004bcc;
  --lblue:#1a6ef7;--text:#e8eaf0;--muted:#8892a4;--white:#ffffff;
  --red:#dc3232;--orange:#ff8c00;--green:#00dc78;--yellow:#ffc800;
  --magenta:#c800c8;--border:#1e2240;
  --font-head:'Courier New','Consolas',monospace;
  --font-body:'Segoe UI','Helvetica Neue',Arial,sans-serif;
  --radius:8px;--shadow:0 4px 32px rgba(0,212,255,0.07);
}
...
.site-header{background:linear-gradient(180deg,#000010 0%,var(--bg2) 100%);
  border-bottom:2px solid var(--cyan);padding:24px 0 0;position:sticky;top:0;
  z-index:100;box-shadow:0 0 40px rgba(0,212,255,0.15)}
.header-inner{max-width:1200px;margin:0 auto;padding:0 24px;
  display:flex;align-items:center;gap:16px;flex-wrap:wrap}
.logo{font-family:var(--font-head);font-size:1.6rem;font-weight:bold;
  color:var(--cyan);text-shadow:0 0 20px rgba(0,212,255,0.5);
  letter-spacing:0.05em;flex-shrink:0}
.logo span{color:var(--white)}
.tagline{color:var(--muted);font-size:0.8rem;font-family:var(--font-head);
  letter-spacing:0.1em}
.header-search{margin-left:auto;display:flex;gap:8px;align-items:center}
.search-input{background:var(--bg3);border:1px solid var(--border);
  color:var(--text);padding:8px 14px;border-radius:var(--radius);
  font-size:0.9rem;width:220px;transition:border-color 0.2s}
.search-input:focus{outline:none;border-color:var(--cyan);
  box-shadow:0 0 8px rgba(0,212,255,0.2)}
.cat-filters{display:flex;gap:8px;padding:14px 24px;max-width:1200px;
  margin:0 auto;overflow-x:auto;scrollbar-width:none}
.cat-btn{background:var(--bg3);border:1px solid var(--border);
  color:var(--muted);padding:5px 14px;border-radius:20px;cursor:pointer;
  font-size:0.82rem;white-space:nowrap;transition:all 0.2s;
  font-family:var(--font-body)}
.cat-btn:hover,.cat-btn.active{border-color:var(--cyan);color:var(--cyan);
  background:rgba(0,212,255,0.08)}
main{max-width:1200px;margin:0 auto;padding:32px 24px}
.posts-grid{display:grid;
  grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:24px}
.post-card{background:var(--bg2);border:1px solid var(--border);
  border-radius:var(--radius);overflow:hidden;
  transition:transform 0.2s,box-shadow 0.2s,border-color 0.2s;
  display:flex;flex-direction:column}
.post-card:hover{transform:translateY(-4px);box-shadow:var(--shadow);
  border-color:var(--cyan)}
.post-card .card-img{width:100%;aspect-ratio:1200/627;
  object-fit:cover;display:block}
.post-card .card-img-placeholder{width:100%;aspect-ratio:1200/627;
  background:var(--bg3);display:flex;align-items:center;
  justify-content:center;font-size:3rem}
.card-body{padding:20px;flex:1;display:flex;flex-direction:column}
.card-meta{display:flex;gap:8px;align-items:center;
  margin-bottom:10px;flex-wrap:wrap}
.cat-badge{display:inline-block;padding:3px 10px;border-radius:4px;
  font-size:0.72rem;font-weight:700;letter-spacing:0.05em;
  text-transform:uppercase}
.card-date,.card-source{color:var(--muted);font-size:0.78rem}
...
.read-more{color:var(--cyan);font-size:0.85rem;font-weight:600}
...
.post-title{font-size:2.1rem;font-weight:800;color:var(--white);
  line-height:1.25;margin-bottom:16px;font-family:var(--font-head)}
.post-meta{display:flex;gap:14px;flex-wrap:wrap;
  color:var(--muted);font-size:0.88rem}
.post-meta a{color:var(--cyan)}
.post-body{font-size:1rem;line-height:1.9;color:var(--text)}
.post-body h2{font-size:1.25rem;color:var(--cyan);
...
.fix-box,.prevent-box{background:var(--bg3);border-radius:var(--radius);
...
.share-row{display:flex;gap:12px;margin-top:40px;flex-wrap:wrap}
.share-btn{padding:10px 20px;border-radius:var(--radius);font-weight:600;
...
.site-footer{border-top:1px solid var(--border);padding:32px 24px;
  text-align:center;color:var(--muted);font-size:0.85rem;margin-top:60px}
...
function applyFilters(){
  ...
  cards.forEach(function(card){
    ...
    if(catOk&&searchOk){card.classList.remove('hidden');visible++}
...
def _caption_to_html(caption: str) -> str:
    ...
    SECTION_ICONS = {
        ...
        "HOW TO FIX":       ("🔧 HOW TO FIX (RIGHT NOW)", "fix-box"),
        "HOW TO PREVENT":   ("🛡️ HOW TO PREVENT",   "prevent-box"),
    }
    for line in lines:
        stripped = line.strip()
        matched = False
        for key, (label, box_cls) in SECTION_ICONS.items():
        ...
        if re.match(r"^[①②③④⑤]", stripped) or re.match(r"^[◆◇▸▹▪]", stripped):
            html_parts.append(f"<li>{_html_escape(stripped[1:].strip())}</li>")
        ...
        elif stripped and ("Aryan Kumar" in stripped or "codexploit.in" in stripped
                           or stripped.startswith("—")):
            html_parts.append(
                f'<p style="color:var(--muted);font-size:0.9rem;margin-top:16px;">'
                f'{_html_escape(stripped)}</p>'
            )
        elif stripped:
            html_parts.append(f"<p>{_html_escape(stripped)}</p>")
...
def _make_unique_slug(meta: dict, post_dir: Path) -> str:
    """
    FIX 1: Build a collision-resistant slug.
    
    Strategy: <folder-name>-<6-char-cid-suffix>
    
    This guarantees uniqueness even when:
      - Two posts have very similar titles (same folder name)
      - meta.json has no "slug" key  
      - Posts from different dates have the same short name
    """
    # Use folder name as base (already slugified by the pipeline)
    base = post_dir.name          # e.g. "nvd-advisory-ec82eb"
    
    # Get canonical_id suffix for guaranteed uniqueness
    cid = meta.get("canonical_id", "")
    if cid:
        # canonical_id looks like "url:b662e21c44016b18" or "cve:CVE-2026-1234"
        # strip non-alphanumeric and take last 6 chars
        cid_clean = re.sub(r"[^a-z0-9]", "", cid.lower())[-6:]
    else:
        # Fall back to date portion of the parent directory
        cid_clean = re.sub(r"[^a-z0-9]", "", post_dir.parent.name.lower())[-6:]
    
    # If the folder name already ends in the cid suffix, don't double-add it
    if base.endswith(f"-{cid_clean}") or base.endswith(cid_clean):
        return base
    
    return f"{base}-{cid_clean}"
...
def _load_posts(out_dir: Path) -> list[dict]:
    ...
    for meta_file in sorted(out_dir.rglob("meta.json"), reverse=True):
        ...
        is_flagged = (post_dir / "review.txt").exists()

        # FIX 1: use unique slug
        slug = _make_unique_slug(meta, post_dir)

        # Extra safety: if STILL a collision, append an incrementing counter
        original_slug = slug
        counter = 1
...
<header class="site-header">
  ...
  <div class="cat-filters">{btns}</div>
</header>"""
...
<footer class="site-footer">
  <p>© {year}
    <a href="{BRAND.get('website_url','#')}" target="_blank">{BRAND['brand_name']}</a>
    · {BRAND['person_name']}
  </p>
...
def _page_wrap(title: str, body: str, root_prefix: str = "") -> str:
    """
    root_prefix = ""      for index.html  (at blog root)
    root_prefix = "../../" for posts/<slug>/index.html
    """
...
def _build_index(posts: list[dict], blog_dir: Path) -> None:
    """Generate index.html — the home feed with all post cards."""
    cards_html = ""
    for p in posts:
        if p["flagged"]:
            continue
        ...
        # Image path: index.html is at blog/index.html
        # images are at blog/posts/<slug>/image.png
        # → relative path from index.html = posts/<slug>/image.png
        if p["image"]:
            img_tag = (
                f'<img class="card-img" src="posts/{slug}/{p["image"]}" '
        ...
        try:
            ...
            pub_display = p["pub"][:10] if p["pub"] else ""

        cards_html += f"""
<article class="post-card" data-cat="{cat}" data-title="{_html_escape(p['title'])}">
  <a href="posts/{slug}/index.html">{img_tag}</a>
  <div class="card-body">
    <div class="card-meta">
      {_cat_badge_html(cat)}
    ...
    <h2 class="card-title">
      <a href="posts/{slug}/index.html">{_html_escape(p['title'])}</a>
    </h2>
    <p class="card-excerpt">{_html_escape(excerpt)}</p>
    <div class="card-footer">
      <a class="read-more" href="posts/{slug}/index.html">Read more →</a>
...
<main>
  <div class="posts-grid" id="posts-grid">
    {cards_html if cards_html else '<p style="color:var(--muted);padding:40px;text-align:center">No posts yet. Run the daemon to generate posts.</p>'}
  </div>
  <p class="no-results hidden" id="no-results">No posts match your search.</p>
    ...
    log.info("Generated index.html (%d posts)", published)


def _build_post_page(p: dict, blog_dir: Path) -> None:
    """Generate individual post HTML page at blog/posts/<slug>/index.html."""
    if p["flagged"]:
        return

    slug     = p["slug"]
    post_out = blog_dir / "posts" / slug
    post_out.mkdir(parents=True, exist_ok=True)

    # Copy image into post dir
    img_html = ""
    if p["image"]:
        src = p["dir"] / p["image"]
        ...
        # posts/<slug>/index.html → image is in same folder → just filename
        img_html = (
    ...
    try:
        ...
        pub_display = p["pub"][:19] if p["pub"] else ""

    cat          = p["category"]
    caption_html = _caption_to_html(p["caption"])

    import urllib.parse
    title_enc = urllib.parse.quote(p["title"])
    li_url    = f"https://www.linkedin.com/shareArticle?mini=true&title={title_enc}"
    tw_url    = f"https://twitter.com/intent/tweet?text={title_enc}"
...
<article class="post-page">
  ...
  <p style="margin-top:32px;">
    ...
    # root_prefix="../../" so any assets resolve up to blog root
...
def publish(
    ...
    # FIX 2: Always write .nojekyll — without this GitHub Pages runs Jekyll
    # which ignores _underscore folders and may mangle HTML files
    (blog_dir / ".nojekyll").write_text("", encoding="utf-8")
